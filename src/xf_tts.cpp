...
const char* fileName ="/home/archer/音乐/voice.wav";
const char* playPath ="play /home/archer/音乐/voice.wav";
/* wav音频头部格式 */
...
/**
* make text to wav flie
**/
void makeTextToWav(const char* text,const char* filename)
{
	int         ret                  = MSP_SUCCESS;
	const char* login_params         = "appid = 5b095934, work_dir = .";//登录参数,appid与msc库绑定,请勿随意改动
	/*
	* rdn:           合成音频数字发音方式
	* volume:        合成音频的音量
	* pitch:         合成音频的音调
	...
	*/
	...
	//char* text                 = "大家好，我是由南华大学创元团队制造的3D打印机器人，我全身的大部分部件是由3D打印机打印而成的。"; //合成文本
...
{
	const char* start = "成功调用科大讯飞在线语音合成";
	makeTextToWav(start , fileName);
	playWav();

	ros::init(argc,argv,"xf_tts_node");
	ros::NodeHandle nd;
	ros::Subscriber sub = nd.subscribe("/voice/xf_tts_topic", 5 , topicCallBack);
	ros::spin();
